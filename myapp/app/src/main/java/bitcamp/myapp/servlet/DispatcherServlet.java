...
//      controllers.add(new MemberController(memberDao));
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      //URL 요청을 처리할 requestHandler를 찾는다.
      ...
      // 페이지 컨트롤러에서 오류가 발생하면 오류 페이지로 포워딩한다.
  ...
  private void findComponents(File dir, String pakageName) throws Exception {
    ...
    for (File file : files) {
      if (file.isFile()) {
        ...
        findComponents(file, pakageName + file.getName());
      }
    }
  }
    ...
    for (Object controller : controllers) {
      ...
      for (Method m : methods) {
        ...
        if (requestMapping != null) {
          requestHandlerMap.put(requestMapping.value(), new RequestHandler(m, controller));
        }
      }
    }
  }
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (methodParam.getType() == HttpServletRequest.class
        ...
        if (requestParam != null) {
          // 파라미터 타입이 primitive data type, String, Date일 경우
  ...
  private Object createValueObject(Class<?> type, HttpServletRequest request) throws Exception {
    // request handler가 원하는 값이 Domain 객체라면,
    // 도메인 객체를 생성한 후
    ...
    for (Method setter : methods) {
      ...
      String requestParameterValue = request.getParameter(propName);

      // 7) 도메인 객체의 프로퍼티 이름과 일치하는 요청 파라미터 값이 있다면 객체에 저장한다.
      if (requestParameterValue != null) {
        // setter 메서드 파라미터 타입을 알아낸다
        Class<?> setterParameterType = setter.getParameters()[0].getType();
        // setter를 호출한다
        setter.invoke(obj, valueOf(requestParameterValue, setterParameterType));
      }
    }
    return obj;
  }
